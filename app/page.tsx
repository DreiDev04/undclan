...
export default function ClanLootSystem() {
  ...
  const clearHistory = () => {
    if (
      confirm(
        "Are you sure you want to delete the entire raffle history? This cannot be undone."
      )
    ) {
      setHistory([]);
    }
  ...
  const handleDownloadHistoryTxt = () => {
    ...
    let content =
      "📜 CLAN LOOT RAFFLE HISTORY 📜\n================================\n\n";
    history.forEach((h) => {
      content += `[${h.displayDate} ${h.displayTime}] ${h.winnerName} won ${
        h.quantityWon
      }x ${h.itemName} (${h.rarity.toUpperCase()})\n`;
    });
    content +=
    ...
    link.href = url;
    link.download = `loot_history_discord_${
  ...
  const handleBackupJson = () => {
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = `clan_loot_FULL_BACKUP_${
      new Date().toISOString().split("T")[0]
    }.json`;
    document.body.appendChild(link);
    link.click();
  ...
  const getRarityColor = (rarity: string) => {
    switch (rarity) {
      case "legendary":
        return "bg-orange-500 text-orange-950 border-orange-500/50";
      case "epic":
        return "bg-purple-500 text-purple-950 border-purple-500/50";
      case "rare":
        return "bg-blue-500 text-blue-950 border-blue-500/50";
      case "uncommon":
        return "bg-green-500 text-green-950 border-green-500/50";
      default:
        return "bg-zinc-500 text-zinc-950 border-zinc-500/50";
    }
  };
    ...
    <div className="min-h-screen w-full bg-[#0a0a0a] text-white p-8 flex flex-col items-center font-sans selection:bg-violet-500/30">
      ...
      <div className="w-full max-w-6xl">
        <Tabs defaultValue="loot" className="w-full">
          <TabsList className="grid w-full grid-cols-4 bg-zinc-500/50 border border-zinc-800 h-12 mb-6 ">
            <TabsTrigger
              value="loot"
          ...
          <TabsContent value="loot" className="space-y-6">
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                  ...
                  <div className="md:col-span-4 space-y-2">
                    ...
                    <Select
                      value={newItemRarity}
                      onValueChange={(v: Rarity) => setNewItemRarity(v)}
                    >
                ...
                <Button
                  onClick={addItem}
                  className="w-full bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-white"
                >
                  <Plus className="w-4 h-4 mr-2" /> Add Loot Item
                </Button>
              </CardContent>
            </Card>
            ...
            <Card className="bg-zinc-950 border-zinc-800 text-zinc-100 min-h-[200px]">
              ...
              <CardContent>
                {lootItems.length === 0 ? (
                  <div className="flex justify-center items-center h-24 text-zinc-500 text-sm">
                    No items added yet
                  </div>
                ) : (
                  <div className="grid gap-3">
                    {lootItems.map((item) => (
                      <div
                        key={item.id}
                        className="flex items-center justify-between p-3 rounded-md bg-zinc-900/50 border border-zinc-800"
                      >
                  ...
                  <CardContent>
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        className="bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-white"
                      >
                ...
                <Card className="bg-zinc-950 border-zinc-800 text-zinc-100">
                  <CardHeader>
                    <CardTitle className="text-base font-semibold flex items-center gap-2">
                      ...
                      <Badge
                        variant="secondary"
                        className="text-xs font-normal bg-zinc-800 text-zinc-400"
                      >
                        Paste from Sheets
                      </Badge>
                    </CardTitle>
                  ...
                  <CardContent className="space-y-4">
                    <Textarea
                      ...
                      className="min-h-[150px] bg-zinc-900 border-zinc-800 font-mono text-sm"
                      value={bulkNames}
              ...
              <Card className="bg-zinc-950 border-zinc-800 text-zinc-100 h-[500px] flex flex-col">
                  ...
                  <CardTitle className="text-base font-semibold flex justify-between items-center">
                    ...
                    {participants.length > 0 && (
                      <Button
                        ...
                        size="sm"
                        onClick={() => {
                          if (confirm("Clear all?")) setParticipants([]);
                        }}
                ...
                <CardContent className="flex-1 overflow-hidden p-0">
                  <ScrollArea className="h-full">
                    {participants.length === 0 ? (
                      <div className="flex flex-col justify-center items-center h-48 text-zinc-500 text-sm gap-2">
                      ...
                      <div className="p-4 grid grid-cols-1 gap-2">
                        {participants.map((p) => (
                          <div
                            key={p.id}
                            className="group flex items-center justify-between p-2 rounded-md bg-zinc-900/50 border border-zinc-800 hover:border-zinc-700 transition-colors"
                          >
          ...
          >
            ...
            <div className="flex-1 relative flex items-center justify-center bg-zinc-950/50 rounded-xl border border-zinc-800 overflow-hidden">
              ...
              <div className="absolute right-12 lg:right-24 top-1/2 -translate-y-1/2 z-20 pointer-events-none filter drop-shadow-xl">
                <div className="w-0 h-0 border-t-[20px] border-t-transparent border-b-[20px] border-b-transparent border-r-[40px] border-r-white transform" />
              </div>
            ...
            {/* RIGHT: CONTROLS */}
            <Card className="w-full lg:w-96 bg-zinc-950 border-zinc-800 flex flex-col h-full text-white">
              <CardHeader className="pb-3 border-b border-zinc-800">
                <CardTitle className="text-lg font-semibold flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <Users className="w-5 h-5" />
                    <span>Entries ({participants.length})</span>
                  </div>
                  <Button
                    variant="ghost"
                    size="icon"
                  ...
                  <div className="p-4 space-y-2">
                    {participants.length === 0 ? (
                      ...
                      participants.map((p, i) => (
                        <div
                          key={p.id}
                          className="flex items-center gap-3 p-2 rounded-md bg-zinc-900/50 border border-zinc-800/50"
                        >
                          <div
                ...
                <div className="flex items-center space-x-2 py-1">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      setRemoveWinner(checked as boolean)
                    }
            ...
            <Card className="bg-zinc-950 border-zinc-800 text-zinc-100">
              ...
              <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <Button
                  ...
                  className="w-full bg-indigo-600 hover:bg-indigo-700 text-white border border-indigo-500/20"
                >
                  <FileText className="w-4 h-4 mr-2" /> Download History (.txt)
                </Button>

                {/* Full Backup */}
                <Button onClick={handleBackupJson} variant={"secondary"}>
              ...
              <div className="space-y-3">
                {history.length === 0 ? (
                  ...
                  history.map((item) => (
                    <div
                      key={item.id}
                      className="group flex items-center justify-between p-4 rounded-lg bg-zinc-900/50 border border-zinc-800 hover:border-zinc-700 transition-all"
                    >
